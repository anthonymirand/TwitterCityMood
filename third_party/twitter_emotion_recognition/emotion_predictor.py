...
class EmotionPredictor:
  ...
  def _get_char_mapping(self):
    with open('{}/models/allowed-chars.json'.format(self.directory), 'rb') as f:
      return json.load(f)
  ...
  def _tweet_to_indices(self, tweets):
    indices = []
    for t in tweets:
      t = HTMLParser.HTMLParser().unescape(t)      # unescape HTML
      t = re.sub(r"http\S+", "", t)                # remove normal URLS
      t = re.sub(r"pic\.twitter\.com/\S+", "", t)  # remove pic.twitter.com URLS
      indices.append([self.char_to_ind.get(char, 1) for char in t])
